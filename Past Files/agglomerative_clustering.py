...
agglomerativeCluster = AgglomerativeClustering(n_clusters=number_of_clusters).fit(data.drop(['type'], axis= 1))
a_cluster = agglomerativeCluster.labels_


#set up color map based on cluster
for row in range(0,number_of_rows):
    color_map.append(color_set[a_cluster[row]])
...
plt.title('t-SNE Dimension-Representation of Agglomerative Clustering')
plt.show()